...
SET file_format = 'EDI_EODP.OPERATIONS.EODP_FI_FILE_FORMAT';
SET bucket_url = 's3://edi-snowflake-mkt/prices_fi/';
...
Create Landing, Temporary, Error, and Final tables for EODP_EQUITY data.
...
/*
Creation of Landing table for EODP_FI data
*/

CREATE TABLE EDI_EODP.OPERATIONS.EODP_FI_INC_LANDING (
...
/*
Creation of Temporary table for EODP_FI data
...
*/
CREATE TABLE EDI_EODP.OPERATIONS.EODP_FI_INC_TEMP (
MIC VARCHAR (13) NOT NULL,
pricefilesymbol CHAR (60),
Isin CHAR (12),
Currency CHAR (3),
...
/*
Creation of Error table for EODP_FI data
The Error table contains rows of files that failed during the process.
*/
CREATE TABLE EDI_EODP.OPERATIONS.EODP_FI_INC_ERROR (
MIC VARCHAR (13) NOT NULL,
pricefilesymbol CHAR (60),
Isin CHAR (12),
Currency CHAR (3),
...
/*
Creation of Final table for EODP_FI data with Column Descriptions
The Final table for EODP_FI data is created, including descriptions for each column.

*/

CREATE TABLE EDI_EODP.PROD.EODP_FI_INC(
...
Query to create a file format for EODP_FI data. Both CSV and TAB separated files are considered as CSV file type. We specify '\t' as the delimiter, where '\t' represents a tab. The file typically has column headers as the first row, so we skip the first row during processing.
Column parser cannot be used simultaneously.

File format for EODP_FI
*/
CREATE FILE FORMAT EDI_EODP.OPERATIONS.EODP_FI_FILE_FORMAT
TYPE = $file_type
...
Creation of an External Stage for EODP_FI data. External Stage is a temporary location in Snowflake where files will be stored temporarily before loading them into your landing table.

The stage references the storage integration and includes the file format.
*/

CREATE STAGE EDI_EODP.OPERATIONS.EODP_FI_STAGE
...
Query to create a Snowpipe for EODP_FI data.
...
CREATE PIPE EDI_EODP.OPERATIONS.EODP_FI_SNOWPIPE
AUTO_INGEST = TRUE AS
COPY INTO EDI_EODP.OPERATIONS.EODP_FI_INC_LANDING
FROM @EDI_EODP.OPERATIONS.EODP_FI_STAGE
FILE_FORMAT = EDI_EODP.OPERATIONS.EODP_FI_FILE_FORMAT;

/*
Pre-requisite 7: Creation of Stream
Queries to create a stream for EODP_FI  on the landing table.

To monitor when the landing table receives new data from the files loaded in the S3 bucket, we create a stream on the EODP_FI landing table.
*/


CREATE STREAM EDI_EODP.OPERATIONS.EODP_FI_STREAM
ON TABLE EDI_EODP.OPERATIONS.EODP_FI_INC_LANDING APPEND_ONLY = TRUE;
...
CREATE SECURE VIEW EDI_EODP.PROD.EODP_FI_INC_PRODUCT(
...
AS
SELECT *
FROM EDI_EODP.PROD.EODP_FI_INC
WHERE
